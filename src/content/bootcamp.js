...
export const weeks = [
  ...
  },
  {
    ...
    mission:
      'از همون تاپیکی که هفته‌ی قبل پر شد، یه نویسنده‌ی دوم می‌خوایم که رکوردها رو با کمترین تأخیر بذاره داخل StarRocks؛ StarRocks یه انبار تحلیلی ستونیه که برای جواب سریع روی حجم بالا ساخته شده و مسیر دومتون از این‌جا رد می‌شه. هم‌زمان Batch Path هم باید راه بیفته: همون فایل‌های Parquet رو با Spark بخونید و ازشون جدول‌های تجمیعی بسازید توی PostgreSQL — چیزی که ما بهش می‌گیم Cube Maker. تا آخر هفته انتظار داریم مسیر دوم جواب بده و Cube Maker شروع شده باشه.\nدر کنارش ابزار ساختتون باید بزرگ بشه: ساختن ایمیج با docker build دستی و کپی‌کردن jar دیگه جواب نمی‌ده — Jib ایمیج رو از دل خود بیلد درمیاره. کیفیت کدم از تعارف درمیاد: SonarQube باید بشینه توی پایپ‌لاین و جلوی کد زیرِ استاندارد رو بگیره.',
  ...
  },
  {
    ...
    mission:
      'این هفته همون‌جاییه که تصمیم‌های هفته‌ی قبل هزینه‌شون رو نشون می‌دن. هر دو مسیر باید تا آخر کامل بشن: Cube Maker باید جدول‌های تجمیعی رو بنشونه توی PostgreSQL، و هر چهار تا تحلیل فاز اول باید از روی داده‌ی واقعی جواب بدن. بازبینی کد توی Gerrit باید عادت باشه نه تشریفات، و پوشش تست باید معنادار باشه نه یه عددی که پایپ‌لاین رو سبز کنه.',
